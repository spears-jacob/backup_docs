locals {
  ...
  all_s3_bucket_names = values(module.s3_bucket_names_constructor.all_s3_bucket_names_map)

  all_db_tables_names = keys(var.db_table_name_to_config_map)
}
...
data "null_data_source" "requested_bucket_keys" {
  ...
  inputs = {
    value = lookup(var.db_table_name_to_config_map, var.db_table_names[count.index]).s3_bucket_key
  }
}
...
data "null_data_source" "all_bucket_keys" {
  ...
  inputs = {
    value = lookup(var.db_table_name_to_config_map, local.all_db_tables_names[count.index]).s3_bucket_key
  }
}
