...
module "bi_red_ddl_manager" {
  ...
  db_table_name_to_config_map = module.bi_red_tables_configuration.db_table_name_to_config_map
}

module "bi_red_tables_configuration" {
  source = "../../configuration"
  env = var.env
}

module "bi_red_init_s3_buckets" {
  source = "../../init-s3-buckets"
  business_unit = var.business_unit
  env = var.env
  flow_category = local.flow_category
