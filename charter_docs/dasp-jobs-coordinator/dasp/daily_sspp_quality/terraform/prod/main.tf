...
terraform {
  backend "s3" {
    ...
    key = "daily-sspp-quality-coordinator-prod/terraform.tfstate"
    region = "us-east-1"
  }
}
...
data "terraform_remote_state" "transient_emr_components" {
  ...
  config = {
    encrypt = true
    bucket = "charter-telemetry-pilot-tf-state"
    key    = "charter-telemetry-pilot-prod/terraform.tfstate"
    region = "us-east-1"
  }
}
...
data "template_file" "coordinator"{
  template = "${file("${path.module}/../../../../py-coordinator/daily_sspp_quality.json")}"
  vars = {
    ...
    logs_bucket_name = "pi-qtm-global-prod-logs"
