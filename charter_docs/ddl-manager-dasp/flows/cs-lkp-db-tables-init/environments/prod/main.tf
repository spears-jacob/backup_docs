...
module "lkp_ddl_manager" {
  ...
  db_table_name_to_config_map = module.lkp_tables_configuration.db_table_name_to_config_map
}

module "lkp_tables_configuration" {
  source = "../../configuration"
  env = var.env
  flow_category = local.flow_category
}

module "lkp_init_s3_bucckets" {
