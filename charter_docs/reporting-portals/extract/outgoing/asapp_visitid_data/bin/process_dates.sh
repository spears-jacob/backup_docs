...
export DaysToLag=${VAR01}

if [ $1 -eq 0 ]; then
  export extract_run_date=`date --date="$RUN_DATE -$DaysToLag day" +%Y-%m-%d`
...
if [ $1 -eq 1 ]; then
  ...
  extract_before_date=`date --date="$extract_run_date -$DaysToLag day" +%Y-%m-%d`
  echo "
...
export END_DATE=`date --date="$extract_run_date +2 day" +%Y-%m-%d`
