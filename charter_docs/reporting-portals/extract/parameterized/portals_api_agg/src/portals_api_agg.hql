...
INSERT OVERWRITE TABLE asp_api_agg partition(denver_date)
SELECT
  grouping_id,
  CASE WHEN (grouping_id & 1) != 0 THEN hour_denver ELSE 'All Hours' END AS hour_denver,
  CASE WHEN (grouping_id & 2) != 0 THEN minute_group ELSE 'All Minutes' END AS minute_group,
  CASE WHEN (grouping_id & 4) != 0 THEN application_name ELSE 'All Applications' END AS application_name,
  CASE WHEN (grouping_id & 8) != 0 THEN mso ELSE 'All MSOs' END AS mso,
  CASE WHEN (grouping_id & 128) != 0 THEN cust_type ELSE 'All Customer Types' END AS cust_type,
  CASE WHEN (grouping_id & 16) != 0 THEN api_category ELSE 'All Categories' END AS api_category,
  CASE WHEN (grouping_id & 32) != 0 THEN api_name ELSE 'All API' END AS api_name,
  CASE WHEN (grouping_id & 256) != 0 THEN stream_subtype ELSE 'All Stream Subtypes' END AS stream_subtype,
  CASE WHEN (grouping_id & 512) != 0 THEN current_page_name ELSE 'All Page Names' END AS current_page_name,
  metric_name,
  metric_value,
  ...
  CASE WHEN (grouping_id & 1024) != 0 THEN technology_type ELSE 'All Tech Types' END AS technology_type,
  denver_date
FROM
  (
  SELECT
