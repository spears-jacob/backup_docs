...
export report_table_where_value1=${32}
export report_table_where_value2=${33}
export report_table_where_value3=${34}
export csv_file_select=${35}
...
if [ "$RUN_DATE" != "" ]; then
  ...
  SELECT SUM($report_table_counted_field)
  FROM $report_table
  ...
  "
  data_cnt=$(hive -e "set hive.cli.print.header=false; USE $ENVIRONMENT; SELECT SUM($report_table_counted_field) FROM $report_table WHERE $report_table_date_field = '$data_date' AND $report_table_where_field1 = '$report_table_where_value1' AND $report_table_where_field2 = '$report_table_where_value2' AND $report_table_where_field3 = '$report_table_where_value3';");
...
if [ ${data_cnt[0]} -gt 0 ]; then
  ...
  export SecondLineOfCheckingView=$(cat $tab_ck_vw | grep $csv_file_select | grep "${report_table_where_value3}," | sed -n '1 p' | sed 's/"//g')
  ...
  for index in "${!metric_checking_array[@]}"
    ...
    if [ ${#metric_checking_array[@]} -eq 5 ]; then
      export TotalMetricCount="${metric_checking_array[3]}"
    elif [ ${#metric_checking_array[@]} -eq 7 ]; then
      export TotalMetricCount="${metric_checking_array[3]}${metric_checking_array[4]}"
    elif [ ${#metric_checking_array[@]} -eq 9 ]; then
      export TotalMetricCount="${metric_checking_array[3]}${metric_checking_array[4]}${metric_checking_array[5]}"
    fi
