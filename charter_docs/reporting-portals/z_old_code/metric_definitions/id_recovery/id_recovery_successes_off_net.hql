-- QUERY DEFINITIONS FOR id_recovery_successes_off_net
...
SELECT
  ...
  SIZE(COLLECT_SET(CASE WHEN message__name IN ('Recover-final1.nbtm', 'Recover-final2.nbtm') THEN visit__visit_id ELSE NULL END)) as id_recovery_successes_off_net
FROM net_events
...
SELECT
  ...
  CAST(NULL AS INT) as id_recovery_successes_off_net
...
SELECT
  'SMB' as portal_platform,
  'L-TWC' as legacy_footprint,
  size(collect_set(if(state__view__current_page__page_name = 'bc > forgot username> email sent',visit__visit_id,NULL))) - size(collect_set(if(state__view__current_page__page_name = 'bc > forgot username> email sent' and (visit__isp__isp = 'rr.com' or visit__isp__isp = 'twcable.com' or visit__isp__isp = 'twcbiz.com'),visit__visit_id,NULL))) as id_recovery_successes_off_net
