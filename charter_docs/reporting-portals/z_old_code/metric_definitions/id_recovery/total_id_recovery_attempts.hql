-- QUERY DEFINITIONS FOR total_id_recovery_attempts
...
SELECT
  ...
  (SIZE(COLLECT_SET(CASE WHEN message__name IN ('Recover-1.btm') THEN visit__visit_id ELSE NULL END))
    ...
    + (SIZE(COLLECT_SET(CASE WHEN message__name IN ('Recover-1.nbtm') THEN visit__visit_id ELSE NULL END))
      - SIZE(COLLECT_SET(CASE WHEN message__name IN ('Recover-noID.nbtm') THEN visit__visit_id ELSE NULL END))) as total_id_recovery_attempts
FROM net_events
...
SELECT
  ...
  SIZE(COLLECT_SET(IF(state__view__current_page__page_name = 'cla > retrieve username > lookup account',visit__visit_id,NULL))) as total_id_recovery_attempts
...
SELECT
  ...
  MAX(IF(metric = 'username_recovery_attempts',value,NULL)) as total_id_recovery_attempts
...
SELECT
  ...
  'L-CHTR' as legacy_footprint,
  size(collect_set(if(message__name = 'Username Recovery Next' and message__category = 'Custom Link', visit__visit_id,NULL))) as total_id_recovery_attempts
FROM sbnet_events
WHERE partition_date_utc >= DATE_SUB(current_date,2)
  AND epoch_converter(message__timestamp*1000, 'America/Denver') = DATE_SUB(current_date,1)
;
...
SELECT
  'SMB' as portal_platform,
  'L-TWC' as legacy_footprint,
  size(collect_set(if(state__view__current_page__page_name = 'bc > forgot username > step 1' and state__view__previous_page__page_name like 'my account%',visit__visit_id,NULL))) as total_id_recovery_attempts
...
SELECT
  'SMB' as portal_platform,
  'L-BHN' as legacy_footprint,
  CAST(NULL AS INT) as total_id_recovery_attempts
