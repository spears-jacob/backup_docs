-- QUERY DEFINITIONS FOR password_reset_successes_off_net
...
SELECT
  ...
  SIZE(COLLECT_SET(CASE WHEN message__name IN ('Reset-final.nbtm') THEN visit__visit_id ELSE NULL END)) as password_reset_successes_off_net
FROM net_events
WHERE partition_date = DATE_SUB(current_date,1)
;

-- L-TWC Query
  -- Note Tracked for TWC RES
SELECT
  'RES' as portal_platform,
  'L-TWC' as legacy_footprint,
  CAST(NULL AS INT) as password_reset_successes_off_net
...
SELECT
  'SMB' as portal_platform,
  'L-CHTR' as legacy_footprint,
  size(collect_set(if(state__view__current_page__page_name = 'bc > forgot password > change password confirmation',visit__visit_id,NULL))) - size(collect_set(if(state__view__current_page__page_name = 'bc > forgot password > change password confirmation' and (visit__isp__isp = 'rr.com' or visit__isp__isp = 'twcable.com' or visit__isp__isp = 'twcbiz.com'),visit__visit_id,NULL))) as password_reset_successes_off_net
