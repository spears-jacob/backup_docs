-- QUERY DEFINITIONS FOR password_reset_attempts_on_net
...
SELECT
  ...
  SIZE(COLLECT_SET(CASE WHEN message__name IN ('Reset-1.btm') THEN visit__visit_id ELSE NULL END))
    + SIZE(COLLECT_SET(CASE WHEN message__name IN ('Reset-1.bam') THEN visit__visit_id ELSE NULL END)) as password_reset_attempts_on_net
FROM net_events
...
SELECT
  ...
  CAST(NULL AS INT) as password_reset_attempts_on_net
...
SELECT
  ...
  'L-CHTR' as legacy_footprint,
  size(collect_set(if(message__name = 'Password Recovery Next' and message__category = 'Custom Link' and visit__isp__isp RLIKE 'charter.*', visit__visit_id, NULL))) as password_reset_attempts_on_net
FROM sbnet_events
WHERE partition_date_utc >= DATE_SUB(current_date,2)
  AND epoch_converter(message__timestamp*1000, 'America/Denver') = DATE_SUB(current_date,1)
;
...
SELECT
  'SMB' as portal_platform,
  'L-CHTR' as legacy_footprint,
  size(collect_set(if(state__view__current_page__page_name = 'bc > forgot password > step 1' and state__view__previous_page__page_name like 'my account%' and (visit__isp__isp = 'rr.com' or visit__isp__isp = 'twcable.com' or visit__isp__isp = 'twcbiz.com'),visit__visit_id,NULL))) as password_reset_attempts_on_net
