-- QUERY DEFINITIONS FOR cancelled_service_appointments
...
SELECT
  ...
  SIZE(COLLECT_SET(IF(message__name IN ('cancel-success-page','cancel-success-page_STVA'), visit__visit_id, NULL))) as recheduled_service_appoint_count
FROM net_events
...
SELECT
  ...
  SIZE(COLLECT_SET(IF(state__view__current_page__sub_section LIKE '%cancel%',visit__visit_id,NULL))) as recheduled_service_appoint_count
...
SELECT
  ...
  'L-BHN' as legacy_footprint,
  size(collect_set(if(message__name = 'Confirm Cancel Appointment' and state__view__current_page__section = 'Appointment Tracking',visit__visit_id,NULL))) as recheduled_service_appoint_count
FROM sbnet_events
WHERE partition_date_utc >= DATE_SUB(current_date,2)
  AND epoch_converter(message__timestamp*1000, 'America/Denver') = DATE_SUB(current_date,1)
;
...
SELECT
  'SMB' as portal_platform,
  'L-BHN' as legacy_footprint,
  size(collect_set(if(state__view__current_page__elements__name = 'my account > support > cancel appointment submit',visit__visit_id,NULL))) as recheduled_service_appoint_count
FROM sbnet_events
WHERE partition_date_utc >= DATE_SUB(current_date,2)
  AND epoch_converter(message__timestamp*1000, 'America/Denver') = DATE_SUB(current_date,1)
;
