-- QUERY DEFINITIONS FOR total_id_creation_attempts_on_net
...
SELECT
  ...
  (SIZE(COLLECT_SET(CASE WHEN state__view__current_page__name IN ('my-account.create-id-1.bam') THEN visit__visit_id ELSE NULL END))
    ...
    as total_id_creation_attempts_on_net
FROM net_events
...
SELECT
  ...
  SUM(IF(array_contains(message__feature__name,'Custom Event 1') and visit__connection__network_status='cla 3.0:in home',1,0)) as total_id_creation_attempts_on_net
...
SELECT
  ...
  CAST(NULL AS INT) as total_id_creation_attempts_on_net
...
SELECT
  ...
  'L-CHTR' as legacy_footprint,
  size(collect_set(if(message__name = 'Create Account Register' and visit__isp__isp LIKE 'charter%',visit__visit_id,NULL))) as total_id_creation_attempts_on_net
FROM sbnet_events
WHERE partition_date_utc >= DATE_SUB(current_date,2)
  AND epoch_converter(message__timestamp*1000, 'America/Denver') = DATE_SUB(current_date,1)
;
...
SELECT
  'SMB' as portal_platform,
  'L-TWC' as legacy_footprint,
  size(collect_set(if(array_contains(message__name,'CLA: Create Your Account') and state__view__current_page__page_id like 'https://registration.timewarnercable.com/businessclass%' and (visit__isp__isp = 'rr.com' or visit__isp__isp = 'twcable.com' or visit__isp__isp = 'twcbiz.com'),visit__visit_id,NULL))) as total_id_creation_attempts_on_net
