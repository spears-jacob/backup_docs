-- QUERY DEFINITIONS FOR sub_users_created_off_net
...
SELECT
  ...
  sum(if(state__view__current_page__elements__name = 'my account > users > add user save' and state__view__current_page__page_name = 'my account > users',1,0)) - sum(if(state__view__current_page__elements__name = 'my account > users > add user save' and state__view__current_page__page_name = 'my account > users' and (visit__isp__isp = 'rr.com' or visit__isp__isp = 'twcable.com' or visit__isp__isp = 'twcbiz.com'),1,0)) as sub_users_created_off_net
FROM sbnet_events
WHERE partition_date_utc >= DATE_SUB(current_date,2)
  AND epoch_converter(message__timestamp*1000, 'America/Denver') = DATE_SUB(current_date,1)
;
...
SELECT
  ...
  CAST(NULL AS INT) as sub_users_created_off_net
FROM sbnet_events
WHERE partition_date_utc >= DATE_SUB(current_date,2)
  AND epoch_converter(message__timestamp*1000, 'America/Denver') = DATE_SUB(current_date,1)
;
