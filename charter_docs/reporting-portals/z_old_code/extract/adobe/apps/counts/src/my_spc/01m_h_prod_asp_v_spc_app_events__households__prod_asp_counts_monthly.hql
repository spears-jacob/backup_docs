...
DROP TABLE IF EXISTS ${env:TMP_db}.asp_counts_monthly_counts_households_columns;
CREATE TEMPORARY TABLE ${env:TMP_db}.asp_counts_monthly_counts_households_columns AS
    SELECT
        CASE
...
PARTITION(unit,platform,domain,company,year_month_denver,source_table)
    ...
    FROM (SELECT  company,
          ...
          FROM ${env:TMP_db}.asp_counts_monthly_counts_households_columns
        ) as derived_table
