...
DROP TABLE IF EXISTS ${env:TMP_db}.asp_counts_monthly_counts_instances_columns;
CREATE TEMPORARY TABLE ${env:TMP_db}.asp_counts_monthly_counts_instances_columns AS
    SELECT
        'L-CHTR' as company,
...
PARTITION(unit,platform,domain,company,year_month_denver,source_table)
    ...
    FROM (SELECT  company,
          ...
          FROM ${env:TMP_db}.asp_counts_monthly_counts_instances_columns
        ) as derived_table
