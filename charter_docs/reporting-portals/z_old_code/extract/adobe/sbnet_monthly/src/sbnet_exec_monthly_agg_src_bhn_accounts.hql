...
SELECT
  --Identifying data
  company AS company,
  'bhn_accounts' AS source_file,
  --HHs
  CAST(NULL as INT) AS hhs_logged_in, --HHs not tracked in BHN Adobe
  ...
  --Auth
  CAST(NULL as INT) AS total_login_attempts, --Tracked through SSO Profile DB
  CAST(NULL as INT) AS total_login_successes, --Tracked through SSO Profile DB
  --Self-Help Page
  ...
  IF(site_id='005' and role='Administrator',CAST(REGEXP_REPLACE(REGEXP_REPLACE(new_accounts,'"',''),',','') as bigint),NULL) AS total_new_accounts_created, --Sent from BHN BI
