...
-- performs percent calculations for percent_success_new_ids_charter_off_net metric
...
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
-- Begin percent_success_new_ids_charter_off_net metric calculation insert by company --
...
'percent_success_new_ids_charter_off_net' AS metric
FROM
(SELECT
    tab.company,
    '${env:YEAR_MONTH}' AS year_month,
    ...
    CASE WHEN (den1.value IS NOT NULL 
      ...
      AND num1.value <> '0')
        THEN SUM(den1.value + den2.value)
    ...
    LEFT JOIN
        ...
        WHERE
        metric = 'new_ids_charter_count_off_net'
    ...
    LEFT JOIN
        ...
        net_products_agg_monthly_tableau
        WHERE
        metric = 'new_ids_charter_count_on_net'
        AND year_month = '${env:YEAR_MONTH}'
        AND company <> 'Total Combined'
        GROUP BY
        year_month,
    ...
    ON den2.company = tab.company
        AND den2.year_month = tab.year_month
    WHERE 
    tab.year_month = '${env:YEAR_MONTH}'
    AND tab.metric = 'percent_success_new_ids_charter_off_net'
...
SELECT '*****-- End percent_success_new_ids_charter_off_net metric calculation insert by company --*****'
;

-- End percent_success_new_ids_charter_off_net metric calculation insert by company --
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
-- Begin percent_success_new_ids_charter_off_net metric calculation insert by company --
...
SELECT '*****-- End percent_success_new_ids_charter_off_net metric calculation insert by Total Combined --*****'
;

-- End percent_success_new_ids_charter_off_net metric calculation insert by Total Combined --
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------