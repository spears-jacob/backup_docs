...
-- performs percent calculations for percent_total_create_id_count_all metric
...
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
-- Begin percent_total_create_id_count_all metric calculation insert by company --
...
'percent_total_create_id_count_all' AS metric
FROM
(SELECT
    tab.company,
    '${env:YEAR_MONTH}' AS year_month,
    CASE WHEN (num1.value IS NOT NULL 
      AND num1.value <> '0'
      AND num2.value IS NOT NULL 
      AND num2.value <> '0'
      AND den1.value IS NOT NULL 
      AND den1.value <> '0')
        THEN SUM(num1.value + num2.value)
    ...
    ON num2.company = tab.company
        AND num2.year_month = tab.year_month
    LEFT JOIN
        (SELECT
        company,
        year_month,
        ...
        net_products_agg_monthly_tableau
        WHERE
        metric = 'attempts_create_id_count_all'
        AND year_month = '${env:YEAR_MONTH}'
        AND company <> 'Total Combined'
        GROUP BY
        year_month,
    ...
    AND tab.metric = 'percent_total_create_id_count_all'
...
SELECT '*****-- End percent_total_create_id_count_all metric calculation insert by company --*****'
;

-- End percent_total_create_id_count_all metric calculation insert by company --
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
-- Begin percent_total_create_id_count_all metric calculation insert by company --
...
SELECT '*****-- End percent_total_create_id_count_all metric calculation insert by Total Combined --*****'
;

-- End percent_total_create_id_count_all metric calculation insert by Total Combined --
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------