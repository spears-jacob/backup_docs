...
DROP TABLE IF EXISTS ${env:TMP_db}.asp_venona_counts_weekly_counts_visits_columns;
CREATE TEMPORARY TABLE ${env:TMP_db}.asp_venona_counts_weekly_counts_visits_columns AS
    SELECT
...
PARTITION(unit,platform,domain,company,week_ending_date_denver,source_table)
    ...
    FROM (SELECT  company,
          ...
          FROM ${env:TMP_db}.asp_venona_counts_weekly_counts_visits_columns
        ) as derived_table
