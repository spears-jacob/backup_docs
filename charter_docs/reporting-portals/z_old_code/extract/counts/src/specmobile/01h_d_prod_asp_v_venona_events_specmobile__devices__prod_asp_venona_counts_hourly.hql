...
DROP TABLE IF EXISTS ${env:TMP_db}.asp_venona_counts_hourly_counts_devices_columns;
CREATE TEMPORARY TABLE ${env:TMP_db}.asp_venona_counts_hourly_counts_devices_columns AS
    SELECT
...
PARTITION(unit,platform,domain,company,date_denver,source_table)
    ...
    FROM (SELECT  company,
          ...
          FROM ${env:TMP_db}.asp_venona_counts_hourly_counts_devices_columns
        ) as derived_table
