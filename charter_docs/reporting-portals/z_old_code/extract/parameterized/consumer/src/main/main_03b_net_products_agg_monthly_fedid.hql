...
-------------------------------------------------------------------------------
-- Drop and rebuild net_fed_ID_ temp table --

DROP TABLE IF EXISTS ${env:TMP_db}.net_fed_ID_${env:CADENCE} PURGE
;

CREATE TEMPORARY TABLE IF NOT EXISTS ${env:TMP_db}.net_fed_ID_${env:CADENCE}
(
  ${env:pf} STRING,
  source_app STRING,
...
-- END - Drop and rebuild net_fed_ID_cadence temp table --
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
...
INSERT INTO TABLE ${env:TMP_db}.net_fed_ID_${env:CADENCE}

SELECT  ${env:aplh} as ${env:pf},
        source_app AS source_app,
        COUNT(*) AS total_login_attempts,
        SUM(IF(response_code = 'UNIQUE_AUTH', 1, 0)) AS total_successes,
...
WHERE (     partition_date_hour_denver < '2018-06-01_00'
    ...
    AND source_app = 'portals-idp'  -- Resi
...
FROM  (SELECT  'total_login_attempts' as metric,
               ...
               company,
               ${env:pf}
       FROM ${env:TMP_db}.net_fed_ID_${env:CADENCE}
       UNION
