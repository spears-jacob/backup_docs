...
SELECT "Now running asp_sb_agg_05${env:letter}_calc...";
...
INSERT INTO TABLE asp_sb_${env:CADENCE}_agg_calc
...
from       (SELECT ${env:pf},
                   ...
                   company,
                   domain
            from asp_sb_${env:CADENCE}_agg_totals
...
CREATE TEMPORARY TABLE asp_sb_total_combined_calc_${env:percentage_metric_name} AS
...
SELECT "\n\nNow Selecting companies with both numerators and denominators from TEMPORARY TABLE asp_sb_total_combined_calc_${env:percentage_metric_name} \n\n";

SELECt * from asp_sb_total_combined_calc_${env:percentage_metric_name};
...
from       (SELECT n.${env:pf},
                   ...
                   domain,
                   company
            from asp_sb_${env:CADENCE}_agg_totals n
            INNER JOIN asp_sb_total_combined_calc_${env:percentage_metric_name} c
...
INNER JOIN (SELECT d.${env:pf},
                   ...
                   domain,
                   company
            from asp_sb_${env:CADENCE}_agg_totals d
            INNER JOIN asp_sb_total_combined_calc_${env:percentage_metric_name} c
            ...
            GROUP BY d.${env:pf},
                     domain,
                     d.company ) denominator
on  TRIM(numerator.${env:pf}) = TRIM(denominator.${env:pf})
AND TRIM(numerator.company) = TRIM(denominator.company)
;
