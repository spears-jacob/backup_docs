...
FROM (SELECT DISTINCT
       ...
       metric,
       ${env:pf}
     FROM asp_sb_${env:CADENCE}_agg_calc amac
     CROSS JOIN (SELECT DISTINCT company
                 FROM asp_sb_${env:CADENCE}_agg_calc
                 WHERE company <> 'Total Combined' ) co
     ) all_cm
LEFT OUTER JOIN (SELECT company, metric, ${env:pf} from asp_sb_${env:CADENCE}_agg_calc) raw
