...
INSERT OVERWRITE TABLE asp_sbnet_fiscal_monthly_trend_pre PARTITION (company,year_month,metric)
SELECT
    mon.unit,
    mon.value,
    mon.domain,
...
--Year over Year
    ...
    LEFT JOIN ${env:LKP_db}.chtr_fiscal_month fm on mon.year_fiscal_month_denver = fm.fiscal_month
    LEFT JOIN ${env:TMP_db}.sbnet_exec_tableau_metric_lkp names
    ON mon.metric = names.hive_metric
--Month over Month
    LEFT JOIN
        asp_sbnet_fiscal_monthly_trend_lags math
...
INSERT OVERWRITE TABLE asp_sbnet_fiscal_monthly_trend_perc PARTITION (company,year_month,metric)
SELECT
unit,
value,
...
year_month,
metric
FROM asp_sbnet_fiscal_monthly_trend_pre pre
LEFT JOIN ${env:LKP_db}.chtr_fiscal_month fm on pre.year_month = fm.fiscal_month
