...
AND ((metric IN ( 'login_attempts_adobe',
                ...
                'view_statement',
                'one_time_payment_updated',
                'one_time_payment_w_autopay_updated',
                'auto_payment_confirm_updated',
                'reschedules',
                'cancels',
                'new_admin_accounts_created',
                'sub_acct_created',
                'support_pvs',
...
DROP TABLE IF EXISTS asp_operational_daily_staging_weighted_averaging;
...
FROM  (SELECT
          ...
          CASE WHEN state__view__current_page__page_name = 'Overview' THEN 'home-authenticated'
               WHEN state__view__current_page__page_name = 'Login' THEN 'home-unauth'
               ELSE 'other'
          END as auth_unauth_page,
          visit__visit_id as unique_visits,
          MAX(floor((state__view__current_page__page_load_time_ms)/1000)) as pg_load_sec
        FROM asp_v_sbnet_events
        WHERE (partition_date_utc  >= DATE_SUB("${env:RUN_DATE}",${hiveconf:MetricLag}))
            AND state__view__current_page__page_name in ('Login','Overview')
            AND message__category = 'Page View'
            AND state__view__current_page__page_load_time_ms > 0
        GROUP BY  ${env:ape} ,
                  CASE WHEN state__view__current_page__page_name = 'Overview' THEN 'home-authenticated'
                       WHEN state__view__current_page__page_name = 'Login' THEN 'home-unauth'
                       ELSE 'other'
...
FROM  (SELECT
          ...
          floor((state__view__current_page__page_load_time_ms)/1000) as hot_pg_load_sec
        FROM asp_v_sbnet_events
        WHERE partition_date_utc >= DATE_SUB("${env:RUN_DATE}",${hiveconf:MetricLag})
            AND state__view__current_page__page_name in ('Login','Overview')
            AND message__category = 'Page View'
        ...
        GROUP BY  ${env:ape} ,
                  CASE WHEN state__view__current_page__page_name = 'Overview' THEN 'home-authenticated'
                       ...
                       ELSE 'other'
                  END,
                  (floor((state__view__current_page__page_load_time_ms)/1000))
      ) dictionary
GROUP BY  date_Denver,
          auth_unauth_page,
          hot_pg_load_sec
