...
SET TabPrefix=asp_sbnet_euauth_;
...
WHERE partition_date_utc >= "${env:START_DATE}"
  AND partition_date_utc < DATE_ADD("${env:END_DATE}",${hiveconf:RollingLag})
;
...
SELECT  ee.visit__visit_id,
        ...
        MAX(sbe.partition_date_utc) AS max_partition_date
...
WHERE sbe.partition_date_utc >= "${env:START_DATE}"
  AND sbe.partition_date_utc < DATE_ADD("${env:END_DATE}",2*${hiveconf:RollingLag})
...
FROM( SELECT  eei.visit__visit_id,
      ...
      WHERE (sbe2.partition_date_utc >= ("${env:START_DATE}")
        AND  sbe2.partition_date_utc < ("${env:END_DATE}"))
...
FROM  (
         ...
         FROM ( SELECT page_name,
                     ...
                     ${env:apl}
                FROM ${env:TMP_db}.${hiveconf:TabPrefix}events_max_timestamp emt
                ...
                WHERE (date_denver >= ("${env:START_DATE}") AND date_denver < ("${env:END_DATE}"))
                 AND LOWER(message__name) = '${hiveconf:PageName}'
...
FROM  (
         SELECT  'entries_${hiveconf:PageAlias}' AS metric,
          ...
          FROM (  SELECt  count(distinct visit__visit_id) as count_distinct_visits,
                          ${env:apl}
                  FROM ${env:TMP_db}.${hiveconf:TabPrefix}entry_events ee
                  LEFT JOIN ${env:LKP_db}.${env:fm_lkp} fm
                    ON date_denver = fm.partition_date
                  WHERE (date_denver >= ("${env:START_DATE}")
                    AND date_denver < ("${env:END_DATE}"))
                    AND LOWER(message__name) = '${hiveconf:PageName}'
                    AND message__category = '${hiveconf:MsgCategory}'
                 group by ${env:apl}
               ) ne
