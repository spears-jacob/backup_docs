...
INSERT OVERWRITE TABLE asp_app_fiscal_monthly_trend_pre PARTITION (company,year_month,metric)
SELECT
    mon.unit,
    mon.value,
    'app' AS domain,
    NULL AS tableau_display,
--Month over Month
...
--Year over Year
    ...
    mon.metric
    FROM
    asp_app_fiscal_monthly_agg mon
    LEFT JOIN ${env:LKP_db}.${env:fm_lkp} fm 
        ON mon.year_fiscal_month_denver = fm.fiscal_month
--Month over Month
    LEFT JOIN
        asp_app_fiscal_monthly_trend_lags math
    ON math.company = mon.company
        AND math.year_fiscal_month_denver = mon.year_fiscal_month_denver
        AND math.metric = mon.metric
    WHERE
        (fm.partition_date >= ("${env:START_DATE}") AND fm.partition_date < ("${env:END_DATE}"))
    GROUP BY
...
INSERT OVERWRITE TABLE app_fiscal_monthly_trend_perc PARTITION (company,year_month,metric)
SELECT
unit,
value,
...
metric
FROM
asp_app_fiscal_monthly_trend_pre pre
LEFT JOIN ${env:LKP_db}.${env:fm_lkp} fm 
    ON pre.year_month = fm.fiscal_month
WHERE 
    (fm.partition_date >= ("${env:START_DATE}") AND fm.partition_date < ("${env:END_DATE}"))
GROUP BY
company,
