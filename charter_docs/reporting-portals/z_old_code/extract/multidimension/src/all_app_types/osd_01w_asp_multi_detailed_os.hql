...
---------------------------------------------
INSERT OVERWRITE TABLE asp_metrics_detail_os_weekly
PARTITION(unit,domain,company,week_ending_date_denver,source_table)
SELECT
   count(1)            AS value,
   ...
   date_sub(to_date('${env:END_DATE_TZ}'),1)   as week_ending_date_denver,
   CASE WHEN LOWER(visit__application_details__application_name) = 'myspectrum'
...
GROUP BY
   ...
   date_sub(to_date('${env:END_DATE_TZ}'),1),
   CASE WHEN LOWER(visit__application_details__application_name) = 'myspectrum'
        THEN 'asp_v_venona_events_portals_msa'
        WHEN LOWER(visit__application_details__application_name) = 'specnet'
        THEN 'asp_v_venona_events_portals_specnet'
