...
TRUNCATE TABLE asp_metric_pivot_app_adobe PARTITION(platform,domain,company,data_source,year_fiscal_month);
DROP TABLE IF EXISTS ${env:TMP_db}.asp_metric_pull_app_adobe_mytwc PURGE;
...
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.asp_metric_pull_app_adobe_mytwc AS
SELECT
...
INSERT INTO TABLE asp_metric_pivot_app_adobe
PARTITION(platform,domain,company,data_source,year_fiscal_month)
    ...
    FROM (SELECT
          ...
          FROM ${env:TMP_db}.asp_metric_pull_app_adobe_mytwc
        ) as derived_table
