...
SET pivot=asp_metric_pivot_smb_adobe;
...
DROP TABLE IF EXISTS ${env:TMP_db}.asp_metric_pull_smb_adobe_bhn1 PURGE;
DROP TABLE IF EXISTS ${env:TMP_db}.asp_metric_pull_smb_adobe_bhn2 PURGE;
DROP TABLE IF EXISTS ${env:TMP_db}.asp_metric_pull_smb_adobe_twc PURGE;
...
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.asp_metric_pull_smb_adobe_bhn1 AS
SELECT
...
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.asp_metric_pull_smb_adobe_bhn2 AS
SELECT
  '${hiveconf:bhn}' as company,
...
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.asp_metric_pull_smb_adobe_twc AS
SELECT
...
PARTITION(platform,domain,company,data_source,year_fiscal_month)
    ...
    FROM (SELECT
          ...
          FROM ${env:TMP_db}.asp_metric_pull_smb_adobe_bhn1
        ) as derived_table
...
PARTITION(platform,domain,company,data_source,year_fiscal_month)
    ...
    FROM (SELECT
          ...
          FROM ${env:TMP_db}.asp_metric_pull_smb_adobe_bhn2
        ) as derived_table
    LATERAL VIEW EXPLODE (map_column) exploded_table AS metric, metric_value
;
...
PARTITION(platform,domain,company,data_source,year_fiscal_month)
    ...
    FROM (SELECT
          ...
          FROM ${env:TMP_db}.asp_metric_pull_smb_adobe_twc
        ) as derived_table
    LATERAL VIEW EXPLODE (map_column) exploded_table AS metric, metric_value
;
