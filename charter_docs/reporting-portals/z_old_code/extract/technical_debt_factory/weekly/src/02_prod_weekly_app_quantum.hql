...
TRUNCATE TABLE asp_metric_pivot_app_quantum_wk PARTITION(platform,domain,company,data_source,${hiveconf:partition_nm});
DROP TABLE IF EXISTS ${env:TMP_db}.asp_metric_pull_app_quantum1_wk PURGE;
DROP TABLE IF EXISTS ${env:TMP_db}.asp_metric_pull_app_quantum2_wk PURGE;
...
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.asp_metric_pull_app_quantum1_wk AS
...
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.asp_metric_pull_app_quantum2_wk AS
SELECT
CASE
...
INSERT INTO TABLE asp_metric_pivot_app_quantum_wk
PARTITION(platform,domain,company,data_source,${hiveconf:partition_nm})
    ...
    FROM (SELECT
          ...
          FROM ${env:TMP_db}.asp_metric_pull_app_quantum1_wk
        ) as derived_table
...
PARTITION(platform,domain,company,data_source,${hiveconf:partition_nm})
    ...
    FROM (SELECT
          ...
          FROM ${env:TMP_db}.asp_metric_pull_app_quantum2_wk
        ) as derived_table
    LATERAL VIEW EXPLODE (map_column) exploded_table AS metric, metric_value
;
