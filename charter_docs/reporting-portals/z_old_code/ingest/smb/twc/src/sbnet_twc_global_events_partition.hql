...
INSERT OVERWRITE TABLE twcbusglobal_events PARTITION(partition_date_utc, partition_date_hour_utc) 
SELECT 
  MAX(domain),
  MAX(customer),
  ...
  COLLECT_LIST(message__name),
  message__sequence_number,
  message__timestamp,
  MAX(message__triggered_by),
  MAX(message__feature__name),
...
FROM ${env:TMP_db}.sbnet_twc_global_events_no_part
GROUP BY 
  visit__visit_id,
  message__sequence_number,
  message__timestamp,
