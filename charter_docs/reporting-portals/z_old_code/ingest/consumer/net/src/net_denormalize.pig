...
raw_net = LOAD '$TMP.net_raw' USING org.apache.hive.hcatalog.pig.HCatLoader();

-- loading of the tables
map_browser = LOAD '$LKP.net_browser' USING org.apache.hive.hcatalog.pig.HCatLoader();
map_country = LOAD '$LKP.net_country' USING org.apache.hive.hcatalog.pig.HCatLoader();
map_connection_type = LOAD '$LKP.net_connection_type' USING org.apache.hive.hcatalog.pig.HCatLoader();
map_os = LOAD '$LKP.net_os' USING org.apache.hive.hcatalog.pig.HCatLoader();
map_resolution = LOAD '$LKP.net_resolution' USING org.apache.hive.hcatalog.pig.HCatLoader();

-- JOINs to create final relation for denormalization
raw_net = JOIN raw_net BY browser LEFT OUTER, map_browser BY browser_id USING 'replicated';
raw_net = JOIN raw_net BY country LEFT OUTER, map_country BY country_id USING 'replicated';
raw_net = JOIN raw_net BY connection_type LEFT OUTER, map_connection_type BY connection_type_id USING 'replicated';
raw_net = JOIN raw_net BY os LEFT OUTER, map_os BY os_id USING 'replicated';
raw_net = JOIN raw_net BY resolution LEFT OUTER, map_resolution BY resolution_id USING 'replicated';

-- rank does not work agaist big dataset like net_adobe
-- as result, $0 will be NULL
-- in the net_calc.pig script, rank_id is the second in the order by clause, with date_time as the first, which is the correct way to calculate sequence number anyway, however, date_time could be null.
-- CTEC used to orde by UniqueID() to calculate rank_id, so just use unique_id as the second order by in net_calc.pig

--raw_net = rank raw_net;

net_denorm = FOREACH raw_net GENERATE
...
STORE net_denorm INTO '$TMP.net_denorm' USING org.apache.hive.hcatalog.pig.HCatStorer();
