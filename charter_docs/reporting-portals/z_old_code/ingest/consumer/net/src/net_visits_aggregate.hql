...
DROP TABLE IF EXISTS ${env:TMP_db}.net_visit_exclude_visits;
CREATE TABLE ${env:TMP_db}.net_visit_exclude_visits(visit_id STRING);
INSERT INTO TABLE ${env:TMP_db}.net_visit_exclude_visits
  SELECT DISTINCT
    net_visits_agg.visit_id
  FROM
    net_visits_agg
  WHERE
    net_visits_agg.year >= '${hiveconf:LAST_YEAR}'
    AND to_date(net_visits_agg.start_timestamp) >= CAST('${hiveconf:LAST_DATE}' AS DATE);
...
INSERT INTO TABLE ${env:TMP_db}.net_events_daily PARTITION(partition_date)
  ...
  WHERE
    ...
    AND NOT EXISTS (
      SELECT net_visit_exclude_visits.visit_id
      FROM ${env:TMP_db}.net_visit_exclude_visits
      WHERE net_visit_exclude_visits.visit_id = net_events.visit__visit_id
    );
...
FROM (
  ...
  WHERE
    ...
    start_timestamp <= unix_timestamp('${hiveconf:LAST_DATE_TIMESTAMP}')
...
TRUNCATE TABLE ${env:TMP_db}.net_visits_agg_last_date;
INSERT INTO TABLE ${env:TMP_db}.net_visits_agg_last_date SELECT DATE_ADD('${hiveconf:LAST_DATE}', ${hiveconf:NUMBER_OF_DAYS}) FROM ${env:TMP_db}.net_views_agg_last_date LIMIT 1;
