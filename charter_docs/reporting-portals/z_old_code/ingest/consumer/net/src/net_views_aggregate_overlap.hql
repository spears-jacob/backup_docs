...
INSERT INTO TABLE ${env:TMP_db}.net_views_daily PARTITION(partition_date)
  ...
  FROM
    net_events
  INNER JOIN
    ${env:TMP_db}.net_views_existing ON (net_events.state__content__stream__view_id = net_views_existing.view_id)
  WHERE
...
FROM (
  ...
  WHERE
    ...
    start_date_time <= unix_timestamp('${hiveconf:LAST_DATE_TIMESTAMP}') AND 
    net_views_agg.view_id IN (SELECT view_id FROM ${env:TMP_db}.net_views_existing)
...
DROP TABLE IF EXISTS ${env:TMP_db}.net_views_agg;
CREATE TABLE ${env:TMP_db}.net_views_agg LIKE net_views_agg;
INSERT INTO TABLE ${env:TMP_db}.net_views_agg PARTITION(year) SELECT * FROM net_views_agg WHERE year = '${hiveconf:LAST_YEAR}' AND NOT EXISTS (SELECT net_views_existing.view_id FROM ${env:TMP_db}.net_views_existing WHERE net_views_agg.view_id = net_views_existing.view_id);

ALTER TABLE net_views_agg DROP IF EXISTS PARTITION (year='${hiveconf:LAST_YEAR}');
INSERT INTO TABLE net_views_agg PARTITION(year) SELECT * FROM ${env:TMP_db}.net_views_agg;
