...
create table IF NOT EXISTS net_raw_history
(
...
DROP TABLE IF EXISTS ${env:TMP_db}.net_raw;
create table IF NOT EXISTS ${env:TMP_db}.net_raw
( 
  accept_language String,
  browser String,
  browser_height Int,
...
DROP TABLE IF EXISTS ${env:TMP_db}.net_denorm;
create table IF NOT EXISTS ${env:TMP_db}.net_denorm
(
  unique_id String,
  rank_id BigInt,
  accept_language String,
...
DROP TABLE IF EXISTS ${env:TMP_db}.net_calc;
create table IF NOT EXISTS ${env:TMP_db}.net_calc
(
  unique_id String,
  visit_id String,
...
DROP TABLE IF EXISTS ${env:TMP_db}.net_events_no_part;
create table IF NOT EXISTS ${env:TMP_db}.net_events_no_part
(
  domain STRING,
  customer STRING,
...
CREATE TABLE IF NOT EXISTS net_visits_agg 
(
  visit_id STRING COMMENT 'session identifier',
  visitor_id STRING COMMENT 'visitor identifier',
  network_type STRING COMMENT 'wifi/cellular/no connection',
  customer STRUCT
  <
    user_id:STRING, 
    account_id:STRING, 
  ...
  <
    ...
    timezone:STRING,
    status:STRING
  > COMMENT 'Location information details',
  first_time_user                BOOLEAN    COMMENT 'Has the user ever logged into STVA before?',
  total_visits_last_day           INT        COMMENT 'Number of visits in the last 24 hrs?',
  total_visits_last_week          INT        COMMENT 'Number of visits in the last 7 days?',
  total_visits_last_month         INT        COMMENT 'Number of visits in the last month?',
  total_visits_last_3_months      INT        COMMENT 'Number of visits in the last 3 months?',
    total_visits_last_6_months      INT        COMMENT 'Number of visits in the last 6 months?',
    total_visits_last_year          INT        COMMENT 'Number of visits in the last 12 months?',
    last_visit_hours                INT        COMMENT 'duration since last visit end time in hours',
  start_time_of_day               STRING     COMMENT 'morning afternoon night (calculated based on start time of visit  based on local time)',
    end_time_of_day                  STRING     COMMENT 'morning afternoon night (calculated based on end time of visit  based on local time)',
    start_timestamp                 TIMESTAMP  COMMENT 'start time (epoch in ms)',
    end_timestamp                   TIMESTAMP  COMMENT 'end time (utc time ms)',
  visit_duration                  INT        COMMENT 'duration of record',
  total_vod_video_starts              INT        COMMENT 'vod starts',
  total_live_video_starts       INT      COMMENT 'live starts',
  total_download_video_starts         INT COMMENT 'download starts',
  total_vod_view_duration_s       INT COMMENT 'duration vod streaming (seconds)',
  total_live_view_duration_s       INT COMMENT 'duration live streaming (seconds)',
  total_download_view_duration_s     INT COMMENT 'duration download playing (seconds)',
  total_live_buffering_duration_ms   INT COMMENT 'duration live buffering (ms)',
  total_live_buffering_events     INT COMMENT 'count live buffering events',
  vod_buffering_duration_ms       INT COMMENT 'duration vod buffering (ms)',
  vod_buffering_events         INT COMMENT 'count of vod buffering events',
  download_buffering_duration_ms     INT COMMENT 'duration vod buffering (ms)',
  download_buffering_events       INT COMMENT 'count of vod buffering events',
  total_live_stream_failures       INT COMMENT 'count of live stream failures',
  total_vod_stream_failures       INT COMMENT 'count of vod stream failures',
  total_download_failures        INT COMMENT 'count of download failures',
  total_live_stream_restarts       INT COMMENT 'count of live stream restart attempts',
  total_vod_stream_restarts       INT COMMENT 'count of vod stream restart attempts',
  total_download_restarts        INT  COMMENT 'count of download restart attempts',
  total_live_bitrate_upshifts     INT COMMENT 'count of live bitrate upshifts',
  total_live_bitrate_downshifts     INT COMMENT 'count of live bitrate downshifts',
  total_vod_bitrate_upshifts       INT COMMENT 'count of vod bitrate upshifts',
  total_vod_bitrate_downshifts     INT COMMENT 'count of vod bitrate downshifts',
  max_live_bitrate           DOUBLE COMMENT 'max bitrate of record',
  max_vod_bitrate            DOUBLE COMMENT 'max bitrate of record',
  max_download_bitrate         DOUBLE COMMENT 'max bitrate of record',
  min_live_bitrate           DOUBLE COMMENT  'min bitrate of record',
  min_vod_bitrate           DOUBLE COMMENT 'min bitrate of record',
  min_download_bitrate         DOUBLE COMMENT 'min bitrate of record',
  avg_live_bitrate           DOUBLE COMMENT 'avg bitrate of record',
  avg_vod_bitrate            DOUBLE COMMENT 'avg bitrate of record',
  avg_download_bitrate         DOUBLE COMMENT 'avg bitrate of record',
  total_live_startup_events       INT COMMENT 'count of live startup events',
  total_live_startup_duration_ms     INT COMMENT 'duration live startup events(ms)',
  total_vod_startup_events       INT COMMENT 'count of vod startup events',
  total_vod_startup_duration_ms     INT COMMENT 'duration vod startup events(ms)',
  total_download_startup_events     INT COMMENT 'count of download startup events',
  total_download_startup_duration_ms   INT COMMENT 'duration download startup events(ms)',
  total_download_add_to_queues     INT COMMENT 'count of add to queue events',
  total_title_details_views       INT COMMENT 'count of title detail views',
  total_conversions_by_type       Map<STRING,INT> COMMENT 'Map("search" -> 1, "home" -> 2)',
  total_page_views_by_section      Map<STRING,INT>  COMMENT  'Map("home" -> 1, "grid_guide" -> 1)',
  total_feature_usage_counts_by_type  Map<STRING,INT>  COMMENT  'Map("search_attempts" -> 1, "dvr_record_attempts" -> 1)',
  login_errors_service         Map<STRING,INT> COMMENT 'Map("exception A" -> 1, "exception B" -> 2)',
  total_service_exceptions       Map<STRING,INT> COMMENT 'Map("exception A" -> 1, "exception B" -> 2)',
  visit_path              Array<STRING> COMMENT '["home", "search", "vod play"]',
  total_download_completes       INT COMMENT 'total download completes',
  total_download_starts        INT COMMENT 'total download starts',
  total_login_failures        INT COMMENT 'total login failures',
  login_durations            ARRAY<DOUBLE> COMMENT 'login duration'
) COMMENT 'Table contains aggregate details of the visits for .net'
PARTITIONED BY (year STRING) 
row format delimited fields terminated by '\t'
lines terminated by '\n'
stored as orc TBLPROPERTIES('serialization.null.format'='');

DROP TABLE IF EXISTS ${env:TMP_db}.net_visits_agg_last_date;
CREATE TABLE ${env:TMP_db}.net_visits_agg_last_date(partition_date STRING)
row format delimited fields terminated by '\t'
lines terminated by '\n'
stored as textfile;

CREATE TABLE IF NOT EXISTS net_views_agg 
(
  visit_id STRING COMMENT 'session identifier',
  view_id STRING COMMENT 'view identifier',
  view_type STRING,
  customer STRUCT
  <
    user_id:STRING, 
  ...
  <
    id:STRING,
    id_type:STRING 
  > COMMENT 'series information details',
  programmer STRUCT
  < 
    call_sign:STRING,
    id:STRING,
    ...
    >
  > COMMENT 'Programmer information details',
  network_type STRING,
  conversion_type STRING,
  total_views_last_day             INT        COMMENT 'Number of visits in the last 24 hrs?',
  total_views_last_week            INT        COMMENT 'Number of visits in the last 7 days?',
  total_views_last_month           INT        COMMENT 'Number of visits in the last month?',
  total_views_last_3_months        INT        COMMENT 'Number of visits in the last 3 months?',
    total_views_last_6_months        INT        COMMENT 'Number of visits in the last 6 months?',
    total_views_last_year            INT        COMMENT 'Number of visits in the last 12 months?',
    last_view_hours                  INT        COMMENT 'duration since last visit end time in hours',
  start_time_of_day               STRING     COMMENT 'morning afternoon night (calculated based on start time of visit  based on local time)',
    end_time_of_day                  STRING     COMMENT 'morning afternoon night (calculated based on end time of visit  based on local time)',
    start_date_time                 TIMESTAMP  COMMENT 'start time (epoch in ms)',
    end_date_time                   TIMESTAMP  COMMENT 'end time (utc time ms)',
  view_duration_sec                  INT        COMMENT 'duration of views',
  total_live_buffering_duration_ms   INT        COMMENT 'duration buffering (ms)',
  total_buffering_events         INT        COMMENT 'count buffering events',
  buffering_duration_ms         array<string>        COMMENT 'duration of each buffering event in ms',
  total_stream_failures         INT        COMMENT 'count of stream failures',
  total_stream_restarts         INT        COMMENT 'count of restart attempts',
  total_bitrate_upshifts         INT        COMMENT 'count of bitrate upshift',
  total_bitrate_downshifts       INT        COMMENT 'count of bitrate downshift',
  startup_time_ms           INT        COMMENT 'duration of stream startup',
  view_bitrates             map<double,int>        COMMENT 'Map(3->50, 1.5->200)',
  max_bitrate             DOUBLE        COMMENT 'max bitrate of record',
  min_bitrate             DOUBLE        COMMENT 'min bitrate of record',
  avg_bitrate             DOUBLE        COMMENT 'avg bitrate of record'
) COMMENT 'Table contains aggregate details of the visits for .net'
PARTITIONED BY (year STRING) 
row format delimited fields terminated by '\t'
lines terminated by '\n'
...
DROP TABLE IF EXISTS ${env:TMP_db}.net_views_agg_last_date;
CREATE TABLE ${env:TMP_db}.net_views_agg_last_date(partition_date STRING)
row format delimited fields terminated by '\t'
lines terminated by '\n'
stored as textfile;
