...
# check prerequisites, credentials found in common_utilities, and athena_utilities needs to be loaded already to use this script, preferably in ~/.bash_profile
# unless on an EMR
if [[ "$(hostname)" =~ ^(ip-) ]]; then
echo -e "on an EMR\n"
else
...
# automagically loads athena utils functions if the common utilities are already sourced
fn_exists execute_script_in_athena; if [ $? -ne 0 ]; then au; fi;
fn_exists execute_script_in_athena; if [ $? -ne 0 ]; then echo -e "\n\tPlease load athena_utilities in order to use metadata repair, also found in the pi-datalake-user-utilities repository"; kill -INT $$; fi
...
s3patternForZeroByteFiles="^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-5][0-9]:[0-5][0-9]\W+0"
grep -E -h -v "$s3patternForZeroByteFiles" all_objects > all_files_and_intermediates
grep -E -h -v ".hive-staging" all_files_and_intermediates > all_files
