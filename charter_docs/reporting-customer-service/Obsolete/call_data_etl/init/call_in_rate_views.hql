...
--DROP VIEW IF EXISTS ${env:ENVIRONMENT}.steve_v_calls_with_visits;
CREATE VIEW IF NOT EXISTS ${env:ENVIRONMENT}.steve_v_calls_with_visits
AS
...
FROM ${env:ENVIRONMENT}.steve_calls_with_prior_visit a
...
--DROP VIEW IF EXISTS ${env:ENVIRONMENT}.steve_v_visit_rate_4calls;
CREATE VIEW IF NOT EXISTS dev.steve_v_visit_rate_4calls
AS
SELECT *
FROM (
SELECT
...
FULL JOIN (SELECT call_end_date_utc as call_date,                               -- Query pulls all handled calls and summarizes by date
                  ...
                  count(distinct (case when lower(dev.aes_decrypt256(account_number)) !='unknown' AND enhanced_account_number = 0 then call_inbound_key end)) as handled_acct_calls
            FROM dev.steve_call_data_enhanced
...
FULL JOIN (SELECT call_end_date_utc as call_date,
           ...
           FROM dev.steve_calls_with_prior_visit                                   -- Uses view that limited to handled calls with visits before call
