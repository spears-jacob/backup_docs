...
DROP TABLE IF EXISTS ${env:TMP_db}.steve_page_visits_quantum;
CREATE TABLE ${env:TMP_db}.steve_page_visits_quantum
AS
SELECT distinct
...
FROM ${env:TMP_db}.steve_page_visits_quantum
;

set hive.auto.convert.join=false;
--Step 4: Combine call AND event at page level
DROP TABLE IF EXISTS ${env:TMP_db}.steve_calls_and_events;
CREATE TABLE ${env:TMP_db}.steve_calls_and_events
  AS
  SELECT
         ...
         b.split_sum_desc,
         b.truck_roll_flag
  FROM ${env:TMP_db}.steve_page_visits_quantum a                                                 -- Table containing the spectrum.net, SB.net, and myspectrum app visits for the latest extract timeframe
    INNER JOIN ${env:ENVIRONMENT}.steve_call_data_enhanced b ON ${env:ENVIRONMENT}.aes_decrypt(a.account_number) = ${env:ENVIRONMENT}.aes_decrypt256(b.account_number) --Calls and visits are linked together based on customer account information
...
DROP TABLE IF EXISTS ${env:TMP_db}.steve_calls_with_events24hr_new;
CREATE TABLE ${env:TMP_db}.steve_calls_with_events24hr_new AS
SELECT distinct *
FROM ${env:TMP_db}.steve_calls_and_events
...
INSERT OVERWRITE TABLE ${env:ENVIRONMENT}.steve_calls_with_prior_visit PARTITION (call_end_date_utc)
SELECT  distinct
...
FROM
    (SELECT cd.account_number,
      ...
      FROM ${env:TMP_db}.steve_calls_with_events24hr_new cd
        LEFT JOIN ${env:ENVIRONMENT}.steve_issue_cause_lookup icl
        ...
        LEFT JOIN ${env:ENVIRONMENT}.steve_resolution_lookup l
