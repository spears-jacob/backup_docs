...
CREATE VIEW cs_v_new_disposition_research AS
        SELECT   cd.call_end_date_utc as call_date,
         cd.customer_type,
         cd.product,
...
WHERE cd.segment_handled_flag = true
AND call_end_date_utc >= DATE_SUB(CURRENT_DATE,180+cast(DATE_FORMAT(current_date,"d") as int))                                            -- Identifies handled segments where the agent spoke with the customer
