...
if [ $historical_load == 1 ]; then
  ...
  extract_run_date=$(hive -e "SELECT COALESCE(to_date(MAX(partition_date_utc)),to_date(\"2018-08-24\")) FROM \${env:ENVIRONMENT}.asp_support_venona_events;")
  extract_run_date=`date --date="$extract_run_date -30 day" +%Y-%m-%d`
fi
