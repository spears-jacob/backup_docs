...
RUNDATE=$(date -d "2 days ago" +%Y-%m-%d) #we want to keep updating a fiscal momth's data until 2 days after that month ends
echo $RUNDATE

FISCALMONTH=$(hive -e "SELECT fiscal_month FROM prod_lkp.chtr_fiscal_month WHERE partition_date='$RUNDATE';")
#echo $FISCALMONTH

STARTDATE=$(date -d "40 days ago" +%Y-%m-%d) #Need to be sure our subquery includes all days of the fiscal month. 40 days ensures more than enough
