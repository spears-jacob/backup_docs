...
RUNDATE=$(date -d "8 days ago" +%Y-%m-%d) #we want to keep updating a fiscal momth's data until 2 days after that month ends
#echo $RUNDATE
...
STARTDATE=$(date -d "$RUNDATE 40 days ago" +%Y-%m-%d) #Need to be sure our subquery includes all days of the fiscal month. 40 days ensures more than enough
echo $STARTDATE
