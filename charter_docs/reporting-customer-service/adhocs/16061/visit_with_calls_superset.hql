...
DROP VIEW if exists dev_tmp.cs_experiencing_issues_calls_resolutions;

CREATE VIEW dev_tmp.cs_experiencing_issues_calls_resolutions AS
SELECT *, 
...
JOIN (SELECT DISTINCT call_inbound_key, call_start_timestamp_utc, account_number as call_account_number , resolution_description, cause_description,issue_description
