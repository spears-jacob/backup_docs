...
if [ $historical_load == 1 ]; then
  extract_run_date="2018-06-01"
else
  extract_run_date=$(hive -e "SELECT COALESCE(to_date(MAX(partition_date_utc)),to_date(\"2019-06-01\")) FROM \${env:ENVIRONMENT}.cs_quantum_cid_pageviews;")
