...
\n\n*****\n\nQC Table = red_cs_call_care_data_v QC check = #day+ segment duration\n\n*****\n\n

"
;

insert overwrite table cs_qc
partition (qc_date = '${env:RUN_DATE}',qc_table = 'red_cs_call_care_data_v',qc_check = '#day+ segment duration')
select
...
\n\n*****\n\nQC Table = red_cs_call_care_data_v QC check = max segment duration\n\n*****\n\n

"
;

insert overwrite table cs_qc
partition (qc_date = '${env:RUN_DATE}',qc_table = 'red_cs_call_care_data_v',qc_check = 'max segment duration')
select
call_end_date_utc as data_date
,NULL as data_desc
...
where segment_handled_flag = 1
and segment_duration_minutes >= 1440
and call_end_date_utc between DATE_SUB(CURRENT_DATE(),31) and DATE_SUB(CURRENT_DATE(),2)
group by call_end_date_utc;

SELECT"

\n\n*****\n\nQC Table = red_cs_call_care_data_v QC check = record count\n\n*****\n\n

"
;

insert overwrite table cs_qc
partition (qc_date = '${env:RUN_DATE}',qc_table = 'red_cs_call_care_data_v',qc_check = 'record count')
select
call_end_date_utc as data_date
,NULL as data_desc
,COUNT(1) as numeric_data
...
,NULL as data_desc
,COUNT(1) as numeric_data
from prod.cs_page_view_call_in_rate
...
,NULL as data_desc
,COUNT(1) as numeric_data
from prod.cs_site_section_call_in_rate
where partition_date_utc between '${hiveconf:thirtyone_days}' and '${hiveconf:two_days}'
group by partition_date_utc;
