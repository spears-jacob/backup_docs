...
uaturl = 'https://login.esso-uat.charter.com/nidp/saml2/idpsend?id=pdmi'
unpwurl = 'https://login.esso-uat.charter.com:8443/nidp/app/login?sid=0&sid=0'
tokenurl = 'https://login.esso-uat.charter.com:8443/nidp/app/login?sid=0&sid=0'
...
response = session_vpn.get(uaturl, verify=sslverification)
...
if assertion == '':
    ...
    session_novpn = requests.Session()

    credentials["token"] = input("you are not on VPN please enter VIP-Token: ")

    response = session_novpn.get(uaturl, verify=sslverification)

    response = session_novpn.post(unpwurl, 'option=credential&test=null&Ecom_User_ID={username}&Ecom_Password={password}'.format(**credentials), headers={'Content-Type': 'application/x-www-form-urlencoded'}, verify=sslverification)

    response = session_novpn.post(tokenurl, 'option=credential&test=null&Ecom_User_ID={username}&Ecom_Token={token}'.format(**credentials), headers={'Content-Type': 'application/x-www-form-urlencoded'},verify=sslverification)
