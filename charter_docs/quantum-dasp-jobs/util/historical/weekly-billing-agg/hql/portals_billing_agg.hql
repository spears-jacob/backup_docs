...
-- these metrics are no longer in metric agg:
--   portals_autopay_enroll_radio_toggle,
--   portals_bill_pay_clicks_from_local_nav,
--   portals_bill_delivery_pref_paperless_submit_from_paperless

INSERT OVERWRITE TABLE asp_billing_set_agg PARTITION (label_date_denver, grain)
SELECT
   COAlESCE(application_name, 'All Platforms')      AS application_name,
   COALESCE(mso, 'All Companies')                   AS mso,
   COALESCE(autopay_status, 'All Autopay Statuses') AS autopay_status,
   COALESCE(billing_status, 'All Billing Statuses') AS billing_status,
   metric_name,
   metric_value,
   grouping_id,
...
--      'portals_bill_delivery_pref_paperless_submit_from_paperless_instances', SUM(portals_bill_delivery_pref_paperless_submit_from_paperless),
  ...
  FROM
    ...
    SELECT
      ...
      application_name,
      mso,
      COALESCE(autopay_status, '<null>') AS autopay_status,
      COALESCE(billing_status, '<null>') AS billing_status,
...
--      SUM(portals_bill_delivery_pref_paperless_submit_from_paperless) AS portals_bill_delivery_pref_paperless_submit_from_paperless,
    ...
    GROUP BY
      ...
      application_name,
      mso,
      COALESCE(autopay_status, '<null>'),
      COALESCE(billing_status, '<null>')
    ) sumfirst
  GROUP BY
    label_date_denver,
    application_name,
