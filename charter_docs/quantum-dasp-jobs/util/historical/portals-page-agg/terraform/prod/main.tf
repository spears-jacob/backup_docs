...
module "job" {
  ...
  ### Change per-job ###
  job_name = "portals-page-agg"
  read_db_table_names = [
    "prod.core_quantum_events_sspp"
  ]
  write_db_table_names = [
    "prod_dasp.asp_page_agg",
    "prod_dasp.asp_page_set_counts_agg",
    "prod_dasp.asp_page_set_pathing_agg"
  ]
  step_scripts = {
    "1 - portals_page_agg" = var.job_version
  }
  ...
  extra_properties = {
     azkaban = {
        instance_name = "prod",
        project_name  = "asp_page_agg_tab_refresh_check",
        flow_name     = "portals_page_agg_tab_refresh_check_end"
    }
  }
