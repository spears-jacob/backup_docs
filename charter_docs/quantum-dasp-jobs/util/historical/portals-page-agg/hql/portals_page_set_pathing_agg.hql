...
SELECT
  CASE WHEN (grouping_id & 16) = 0 THEN current_page_name      ELSE 'All Current Pages'  END AS current_page_name,
  CASE WHEN (grouping_id & 8)  = 0 THEN previous_page_name     ELSE 'All Previous Pages' END AS previous_page_name,
  CASE WHEN (grouping_id & 4)  = 0 THEN mso                    ELSE 'All Companies'      END AS mso,
  CASE WHEN (grouping_id & 2)  = 0 THEN application_name       ELSE 'All Applications'   END AS application_name,
...
SELECT
  ...
  CASE WHEN (grouping_id & 4)  = 0 THEN mso                    ELSE 'All Companies'      END AS mso,
  CASE WHEN (grouping_id & 2)  = 0 THEN application_name       ELSE 'All Applications'   END AS application_name,
  CASE WHEN (grouping_id & 1)  = 0 THEN select_action_types_value    ELSE 'All Select Actions'   END AS select_action_types_value,
  grouping_id,
  metric_name,
  metric_value,
  denver_date
