...
export tableau_post_load_refresh=1
export tableau_refresh_check=1
export tableau_workbook="ASPDailyMetricsEmail"
export sleep_for_this_many_seconds=55
export bail_out_after_this_many_loops=66
export filemasks_comma='<datemask>/grain=daily'
export datemask="+%Y-%m-%d"
export hdfs_location='/apps/hive/warehouse/prod.db/venona_set_agg_portals/label_date_denver='
export data_description='Portals Counts Daily'
export TZ=America/Denver
export du_threshold=6000000
export retry_after_min=8
export retry_attempts=36
export use_hours=0
export start_offset="-1 day"
export end_offset="-0 day"

export num_input_params=11

# 0. -  Check that the number of input arguments is correct.
if [ $# -lt $num_input_params ] || [ $# -gt $num_input_params ] || [ -z "${filemasks_comma}" ] || [ -z "${datemask}" ] || [ -z "${hdfs_location}" ] || [ -z "${data_description}" ] || [ -z "${TZ}" ] || [ -z "${du_threshold}" ] || [ -z "${retry_after_min}" ] || [ -z "${retry_attempts}" ] || [ -z "${use_hours}" ] || [ -z "${start_offset}" ] || [ -z "${end_offset}" ] ; then
    echo $0: "
