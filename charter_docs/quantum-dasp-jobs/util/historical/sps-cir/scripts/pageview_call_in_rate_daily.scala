...
try {
  ...
  val ratePageViewTableName = "cs_page_view_call_in_rate_sps"
  val rateSitePageViewTableName = "cs_site_section_call_in_rate_sps"
  ...
  dates.foreach {
    processDate =>
      ...
      spark.catalog.recoverPartitions(s"${environment}.atom_cs_call_care_data_3")
      val callData = {
      ...
      val pageviewCallInRates = {
        visitCalls
          .join(
            totalVisits,
            Seq("application_name", "page_view_url", "partition_date_utc", "legacy_company", "current_app_section", "source_system","sps_flag"),
            "outer"
          )
          .select(
            $"application_name",
