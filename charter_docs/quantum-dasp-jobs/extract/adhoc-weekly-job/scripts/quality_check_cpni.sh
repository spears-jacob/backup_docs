...
export qs="select partition_date_utc, count(1) row_count from $DASP_db.asp_adhoc_cpni_verification where partition_date_utc >= '$START_DATE' group
...
then
  ...
  msg="Please check job adhoc-weekly-job: cpni_verification. Expect ${num_days} days and row_count larger than $app_limit for $RUN_DATE"
