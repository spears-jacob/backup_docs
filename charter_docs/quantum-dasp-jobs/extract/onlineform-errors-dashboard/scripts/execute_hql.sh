...
export LAG_DAYS=10 ##call data is always 2 days in arrears so to capture call data recommended to run it daily for 9 days at a time and run date is always day for before today's date
export LEAD_DAYS=8 ##wrt to date set by lag_days print out
...
echo TIME_ZONE: $TZ
echo START_DATE_TZ: $START_DATE_TZ; ##reviewed to make sure hql is using this variable and not ${hiveconf:START_DATE} since those are not defined here
...
echo "### Running onlineForm Errors Dashboard ...................."
hive -v -f ${ARTIFACTS_PATH}/hql/onlineform_errors_dashboard-${SCRIPT_VERSION}.hql \
