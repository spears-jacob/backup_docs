...
from
  ( Select
        ...
        case when visit__application_details__application_name='SpecNet'
             THEN MAX(visit__account__enc_mobile_account_number) over
                  (partition by visit__application_details__application_name,
                   visit__account__enc_account_number, visit__visit_id)
             ELSE visit__account__enc_mobile_account_number
        end as mobile_acct_enc
...
Select  account_number,
        ...
        MAX(mobile_acct_enc) as mobile_acct_enc,
        date_denver
from (select  account_number,
      ...
      where date_denver = '${hiveconf:START_DATE}'
        and application_name in ('MySpectrum','SpecNet','SMB','SpecMobile')
     ) subquery
group by account_number,
         division,
         division_id,
