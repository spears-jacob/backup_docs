...
if [ "${JOB_STEP_NAME}" == "2_portals_ss_set_agg_call_enrich" ]; then
  ...
  if [ "$check_date" = "01" ]; then
    export RUN_DATE=$(date -d"$RUN_DATE - 10 days" +%Y-%m-%d)
    echo "New month finalized: running monthly for $(date -d"$RUN_DATE" +%B)"
  else
    echo "No new month has finalized; no need to re-run call enrichment"
    exit 0;
  fi
else
