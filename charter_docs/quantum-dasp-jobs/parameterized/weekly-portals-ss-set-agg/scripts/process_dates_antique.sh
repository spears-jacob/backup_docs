...
#checks that grain/cadence is available for reprocessing
if [ $IsReprocess -eq 1 ]; then
  regexp='daily|weekly|monthly|fiscal_monthly';
  if [ $# -lt 4 ] || [ $# -gt 4 ] || [ -z "${1}" ] || [ -z "${2}" ] || [ -z "${3}" ] || [ -z "${4}" ] || [ -z "${CADENCE}" ] || [[ ! $CADENCE =~ $regexp ]]; then

    echo $0: "Usage: process_dates.sh IsReprocess TZ [reprocess_date_table] [reprocess_earliest_date_label]

    For reprocessing, ensure that the GRAIN shell variable is set to one of the following:
      daily
      weekly
      monthly
      fiscal_monthly
