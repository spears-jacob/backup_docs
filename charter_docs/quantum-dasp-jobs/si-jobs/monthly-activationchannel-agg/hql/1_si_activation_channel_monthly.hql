...
select  accountnumber,date_level,user_package,application_group,max(identity_created) as identity_created ,max(login) as login,
...
left join (select distinct acct_id,'1' as login_eligible,application_group from ${env:DASP_db}.si_summary_page_base_master where denver_date between '${hiveconf:START_DATE}' and '${hiveconf:END_DATE}'
and event_case_id in ('mySpectrum_Manual_Login_Success',
'mySpectrum_Resume_Login_Success',
'selfInstall_login_manualSuccess',
'selfInstall_login_autoSuccess')) c on a.acct_id=c.acct_id and a.application_group=c.application_group
left join (select distinct visit__account__enc_account_number as acct_id_ste,visit__application_details__application_name from ${env:GLOBAL_db}.core_quantum_events_sspp
where partition_date_utc between '${hiveconf:START_DATE}' and '${hiveconf:END_DATE}'
    ...
    and message__event_case_id='mySpectrum_selectAction_homeCard_selfInstall_setupEquipment') cqe_ste  on cqe_ste.acct_id_ste=a.acct_id and cqe_ste.visit__application_details__application_name=a.application_group
left join (select distinct visit__account__enc_account_number as acct_id_gts,visit__application_details__application_name from ${env:GLOBAL_db}.core_quantum_events_sspp
where partition_date_utc between '${hiveconf:START_DATE}' and '${hiveconf:END_DATE}'
    and visit__application_details__application_name in ('MySpectrum', 'SelfInstall')
    and visit__account__enc_account_number is not null
    and message__event_case_id='mySpectrum_selectAction_equipment_selfInstall_getStarted') cqe_gts  on cqe_gts.acct_id_gts=a.acct_id  and cqe_gts.visit__application_details__application_name=a.application_group
...
left join (select distinct acct_id,'1' as msa_activation_errors,application_group
from ${env:DASP_db}.si_summary_page_base_master where  denver_date between '${hiveconf:START_DATE}' and '${hiveconf:END_DATE}'
and error_code in ('MSASI-0004','MSASI-0005','MSASI-1003','MSASI-1004','MSASI-1001') 
 ) mae on mae.acct_id=a.acct_id  and a.application_group=mae.application_group
 left join (select distinct acct_id,'1' as web_activation_errors,application_group
from ${env:DASP_db}.si_summary_page_base_master
where  denver_date  between '${hiveconf:START_DATE}' and '${hiveconf:END_DATE}'
and event_case_id in ('selfInstall_error_verifyEquipmentSetup','selfInstall_error_deviceQualification_nonCOAM','selfInstall_error_deviceQualification_blacklistedCOAM','selfInstall_error_deviceQualification_minimallyQualifiedCOAM','selfInstall_error_missingEquipment','selfInstall_error_verifyDeviceOnline','selfInstall_error_verifyDeviceSignal','selfInstall_error_LOBOutage','selfInstall_error_acceptTermsConditions','selfInstall_error_portPhoneTransfer','selfInstall_error_callUs','selfInstall_error_tryAgain')) web  on web.acct_id=a.acct_id
and  a.application_group=web.application_group
)  fn
