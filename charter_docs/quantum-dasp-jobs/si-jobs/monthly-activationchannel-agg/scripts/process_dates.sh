...
export WITHIN_8DAYS_LAST_MONTH_START_DATE=$(date -d "$LAST_MONTH_START_DATE - 0 day" +%F)
export WITHIN_8DAYS_LAST_MONTH_END_DATE=$(date -d "$LAST_MONTH_END_DATE + 7 day" +%F)
export WITHIN_8DAYS_LAST_MONTH_START_DATE_TZ=$(TZ=UTC date -d "$WITHIN_8DAYS_LAST_MONTH_START_DATE `TZ="$2" date -d "$WITHIN_8DAYS_LAST_MONTH_START_DATE" +%Z`" +%Y-%m-%d_%H)
export WITHIN_8DAYS_LAST_MONTH_END_DATE_TZ=$(TZ=UTC date -d "$WITHIN_8DAYS_LAST_MONTH_END_DATE `TZ="$2" date -d "$WITHIN_8DAYS_LAST_MONTH_END_DATE" +%Z`" +%Y-%m-%d_%H)

export PRIOR_60DAYS_LAST_MONTH_START_DATE=$(date -d "$LAST_MONTH_START_DATE - 60 day" +%F)
...
#(prev 8 days) Daily END DATES include appropriate hours for TZ conversion from UTC to local
export PRIOR_8DAYS_START_DATE=$(date -d "$extract_run_date - 8 day" +%F)
export PRIOR_8DAYS_END_DATE="$extract_run_date"
export PRIOR_8DAYS_START_DATE_TZ=$(TZ=UTC date -d "$PRIOR_8DAYS_START_DATE `TZ="$2" date -d "$PRIOR_8DAYS_START_DATE" +%Z`" +%Y-%m-%d_%H)
export PRIOR_8DAYS_END_DATE_TZ=$(TZ=UTC date -d "$PRIOR_8DAYS_END_DATE `TZ="$2" date -d "$PRIOR_8DAYS_END_DATE" +%Z`" +%Y-%m-%d_%H)

export PRIOR_60DAYS_START_DATE=$(date -d "$extract_run_date - 60 day" +%F)
...
#(prev 8 days) Weekly END DATES include appropriate hours for TZ conversion from UTC to local
export WITHIN_8DAYS_WEEK_START_DATE=$(date -d "$CURRENT_WEEK_START_DATE - 0 day" +%F)
export WITHIN_8DAYS_WEEK_END_DATE=$(date -d "$CURRENT_WEEK_END_DATE + 7 day" +%F)
export WITHIN_8DAYS_WEEK_START_DATE_TZ=$(TZ=UTC date -d "$WITHIN_8DAYS_WEEK_START_DATE `TZ="$2" date -d "$WITHIN_8DAYS_WEEK_START_DATE" +%Z`" +%Y-%m-%d_%H)
export WITHIN_8DAYS_WEEK_END_DATE_TZ=$(TZ=UTC date -d "$WITHIN_8DAYS_WEEK_END_DATE `TZ="$2" date -d "$WITHIN_8DAYS_WEEK_END_DATE" +%Z`" +%Y-%m-%d_%H)
...
if   [ $CADENCE == "monthly" ]; then
  ...
  export PRIOR_8DAYS_END_DATE="$WITHIN_8DAYS_LAST_MONTH_END_DATE" ;
  export PRIOR_60DAYS_START_DATE="$PRIOR_60DAYS_LAST_MONTH_START_DATE" ;
  export LABEL_DATE_DENVER="$LAST_MONTH_LABEL_DATE";
  ...
  export PRIOR_60DAYS_START_DATE="$PRIOR_60DAYS_START_DATE" ;
  ...
  export PRIOR_8DAYS_START_DATE="$WITHIN_8DAYS_WEEK_START_DATE" ;
  export PRIOR_8DAYS_END_DATE="$WITHIN_8DAYS_WEEK_END_DATE" ;
  export PRIOR_60DAYS_START_DATE="$PRIOR_60DAYS_START_DATE" ;
...
echo '      {
        ...
        "PRIOR_60DAYS_START_DATE":   "'"$PRIOR_60DAYS_START_DATE"'",		
        "GRAIN": "'"$GRAIN"'",
