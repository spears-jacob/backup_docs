...
SELECT "\n\nFor 1a: ${env:TMP_db}.si_summary_agg_master_weekly_tmp\n\n";

DROP TABLE IF EXISTS ${env:TMP_db}.si_summary_agg_master_weekly_tmp PURGE;
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.si_summary_agg_master_weekly_tmp AS
SELECT
	visit_id,
	device_id,
	acct_id,
...
INSERT INTO TABLE ${env:TMP_db}.si_summary_agg_master_weekly_tmp
SELECT 	*
FROM si_summary_agg_master
...
SELECT *
FROM ${env:TMP_db}.si_summary_agg_master_weekly_tmp
;



--Program 
SELECT "\n\nFor 2a: ${env:TMP_db}.si_work_orders_weekly (Program metrics)\n\n";

DROP TABLE IF EXISTS ${env:TMP_db}.si_work_orders_weekly PURGE;
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.si_work_orders_weekly AS
WITH wo AS (
...
SELECT "\n\nFor 2b: ${env:TMP_db}.si_summary_call_8days_weekly (Program metrics)\n\n";

DROP TABLE IF EXISTS ${env:TMP_db}.si_summary_call_8days_weekly PURGE;
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.si_summary_call_8days_weekly AS
...
SELECT "\n\nFor 2c: ${env:TMP_db}.si_work_orders_calls_weekly (Program metrics)\n\n";

DROP TABLE IF EXISTS ${env:TMP_db}.si_work_orders_calls_weekly PURGE;
CREATE TABLE IF NOT EXISTS ${env:TMP_db}.si_work_orders_calls_weekly AS
SELECT 
...
FROM ${env:TMP_db}.si_work_orders_weekly orders
LEFT JOIN ${env:TMP_db}.si_summary_call_8days_weekly calls
...
FROM (
		SELECT
			...
			orders.encrypted_account_key_256,
			orders.partition_date_denver
		FROM ${env:TMP_db}.si_work_orders_calls_weekly orders
