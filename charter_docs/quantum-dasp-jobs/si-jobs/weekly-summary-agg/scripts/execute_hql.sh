...
execute_hql(){
	...
	hive -S -v -f ${ARTIFACTS_PATH}/hql/1_si_summary_agg_master_weekly-${SCRIPT_VERSION}.hql \
	...
	hive -S -v -f ${ARTIFACTS_PATH}/hql/2_si_dummy_records_weekly-${SCRIPT_VERSION}.hql \
	 -hiveconf START_DATE="${START_DATE}" -hiveconf END_DATE="${END_DATE}" \
	 -hiveconf label_date_denver="${LABEL_DATE_DENVER}" -hiveconf grain="${GRAIN}" \
	 -hiveconf ProcessTimestamp="${processing_started_date_time_denver}" \
	 -hiveconf ProcessUser="${processing_started_by_user}" || { echo "HQL Failure"; exit 101; }
...
# ---------- Execution for 2 weeks ago based on RUN_DATE (Extra execution) ----------
# Since there could be WOs at the end of the week and call care table has a lag up to 3 days (in most of the cases), weekly job must be executed 3 days more
# on next weeek to refresh call metrics within 8 days after WO date
REPROCESS_DAY=$(date --date=$ORIGINAL_RUN_DATE +%A)
MINUS_DAYS=-1
