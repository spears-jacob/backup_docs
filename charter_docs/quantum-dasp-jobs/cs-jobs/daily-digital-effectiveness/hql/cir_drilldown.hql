...
DROP TABLE IF EXISTS cir_drilldown_calls_with_visits_${hiveconf:execid}_${hiveconf:stepid} PURGE;
CREATE TABLE cir_drilldown_calls_with_visits_${hiveconf:execid}_${hiveconf:stepid}
AS
SELECT *
  FROM `${env:CALLVISIT}`
 WHERE call_date BETWEEN '${hiveconf:START_DATE_UTC}' AND '${hiveconf:END_DATE_UTC}'
;

DROP TABLE IF EXISTS cir_drilldown_cqe_enrichment_${hiveconf:execid}_${hiveconf:stepid} PURGE;
CREATE TABLE cir_drilldown_cqe_enrichment_${hiveconf:execid}_${hiveconf:stepid}
AS
SELECT distinct
       partition_date_utc
 ...
 WHERE partition_date_utc BETWEEN DATE_ADD('${hiveconf:START_DATE_UTC}',-1) AND '${hiveconf:END_DATE_UTC}'
...
INSERT overwrite TABLE asp_de_drilldown PARTITION(partition_date_utc)
SELECT state__view__current_page__page_name
       ,state__view__current_page__app_section
       ...
       ,COUNT(DISTINCT CASE WHEN call_inbound_key IS NOT NULL THEN visit__visit_id ELSE NULL END) AS visits_with_call
 ...
 GROUP BY
      ...
      ,state__view__current_page__user_journey
      ,state__view__current_page__user_sub_journey
      ,CASE WHEN b.call_date IS NULL THEN partition_date_utc ELSE b.call_date END
;
