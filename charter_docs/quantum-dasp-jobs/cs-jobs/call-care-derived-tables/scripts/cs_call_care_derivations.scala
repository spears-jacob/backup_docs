...
//      If have a different call table you need to pull data from, put it here and
//      uncomment this one.  That way you can comment out the real one and leave
//      it in place for when you're ready to go back.
 // val callDataTableName = "atom_cs_call_care_data_3_prod_copy"
...
//      If you're checking RED changes, it can be useful to generate the derived
//      tables using both the new and the old table.  In that case, you can uncomment
//      this and the code in the Read in Call Data section, and it will generate
//      both
// val oldCallDataTableName = "atom_cs_call_care_data_3"

//system variables
val environment = sys.env("ENVIRONMENT")
val redDb = s"${environment}_red"
...
// val newCallData = {spark .table(s"$redDb.$callDataTableName")
// val newCallData = {spark .table(s"$daspDb.$callDataTableName")
.where($"call_end_date_utc" >= lit(overwriteDate.toString) && $"call_end_date_utc" <= lit(endDate.toString))
.cache
}

//   The uncommented one is the show() that I find most useful in debugging, but
//   the others have been needed at some point.  Feel free to add more commented
//   ones if you end up needing them.
...
//      If you're making two versions to compare RED changes, uncomment this.
...
spark.table(s"${environment}.core_quantum_events_sspp")
 ...
 .where( $"partition_date_utc" >= overwriteDate.toString &&
  ...
  $"visit_id".isNotNull &&
  $"visit__account__enc_account_number" =!= "dZJho5z9MUSD35BuytdIhg==" &&  // one of these is "UNASSIGNED"
  $"visit__account__enc_account_number" =!= "Qn+T/sa8AB7Gnxhi4Wx2Xg==" &&  // and one of these is "NULL"
...
// when a customer first calls in, we don't have an MSO because they haven't been
// directed yet.  But if a call was TWC at the end of the call, it probably was TWC
// at the beginning.  So we join on call data twice: once at the call level, to get MSO
// and once at the segment level, to get everything else.
  ...
  .where($"enhanced_account_number" === lit(false)).as("portal_atom_join")
    , $"visit__account__enc_account_number" === $"encrypted_padded_account_number_256" // we join on account number and mso, because
     && $"visit_mso" === $"account_agent_mso" , "inner"                         // visit data doesn't have account key
  )
  .join(
  ...
  .where(
     (($"call_start_timestamp_utc" / 1000) - $"received__timestamp") <= "86400"  //Call must be after the visit
       && (($"call_start_timestamp_utc" / 1000) - $"received__timestamp") >= 0)  // and within 24 hours of the visit
  .distinct
  .select(
$"visit__account__enc_account_number"
   , $"visit_id"
   ...
   , $"right_call_data.cause_description".alias("primary_manual_disposition_cause")
   ...
   , $"right_call_data.issue_description".alias("primary_manual_disposition_issue")
   ...
   , $"right_call_data.primary_auto_disposition_cause"
   , $"right_call_data.primary_auto_disposition_issue"
   , $"right_call_data.product"
   , $"right_call_data.record_update_timestamp"
   , $"right_call_data.resolution_description".alias("primary_manual_disposition_resolution")
     ...
     .groupBy(
       ...
       , $"primary_auto_disposition_issue"
        ...
        .where($"visitStart" < ($"call_start_timestamp_utc" / 1000)
          && ($"visitStart" > ($"previous_call_time_utc" / 1000) || $"previous_call_time_utc".isNull)) //This is where we make sure that only one call is counted against a visit
        .withColumn("call_start_div", ($"call_start_timestamp_utc" / 1000))
        .withColumn("issue_description",coalesce($"primary_auto_disposition_issue",$"primary_manual_disposition_issue"))
        .withColumn("cause_description",coalesce($"primary_auto_disposition_cause",$"primary_manual_disposition_cause"))
        .withColumn("resolution_description",coalesce($"primary_manual_disposition_resolution")) //we don't have an auto-resolution, but we're ready for one
        .select(
          $"encrypted_account_key_256"
          , $"encrypted_padded_account_number_256"
          , $"call_inbound_key"
...
////////////////////////////////////////////////////////////////////////////////
//////////   Temporary hack for prodmonthly changes 2022-03-24  ////////////////
////////////////////////////////////////////////////////////////////////////////

/////////////////////////////// Temp hack: Create the three elements of Call-in Rate //////////////////////////////
// The call-in rate table shows, for each set of grouping columns, three things:
//   calls, visits, and calls-with-visit.  Each of those things comes from a different
//   table, so we have to select each of them with grouping columns, and then join
//   them all together
 val cs_visit_prodmonthly_events = (
   quantum_events_portals
    ...
    .withColumn("care_events_app_type", upper($"visit__application_details__application_type"))
    .withColumn("care_events_visit_type", upper($"visit_type"))
    .withColumn("care_events_visit_form", $"visit__device__device_form_factor")
    .groupBy($"care_events_partition_date_utc", $"care_events_app_type", $"care_events_visit_form", $"care_events_visit_type")
    .agg(countDistinct($"visit_id").alias("care_events_authenticated_visits") ).as("cs_visit_prodmonthly")
  )

                 cs_visit_prodmonthly_events.show(10,false)


val cs_prodmonthly_calls_with_prior_visit = (
 steve_calls_with_prior_visit
  ...
  .withColumn("prior_visit_app_type", $"visit__application_details__application_type")
  .withColumn("prior_visit_form", upper($"visit__device__device_form_factor"))
  .groupBy($"prior_visit_call_date", $"prior_visit_app_type", $"prior_visit_form", $"prior_visit_visit_type")
  .agg(countDistinct($"call_inbound_key").alias("prior_visit_calls_with_visit"))
         ).as("cs_visit_prior_visit")


                 cs_prodmonthly_calls_with_prior_visit.show(10,false)


/////////////////////////////// Temp hack: Combine those two elements into cs_prodmonthly_call_in_rate//////////////////////////////
val cs_prodmonthly_cir_table = (
 cs_visit_prodmonthly_events
  .join(cs_prodmonthly_calls_with_prior_visit,
    $"care_events_partition_date_utc" === $"prior_visit_call_date"
    && upper(trim($"care_events_visit_type")) === upper(trim($"prior_visit_visit_type"))
    && upper(trim($"care_events_visit_form")) === upper(trim($"prior_visit_form"))
    && upper(trim($"care_events_app_type")) === upper(trim($"prior_visit_app_type"))
    , "outer")
   .withColumn("call_date", coalesce($"care_events_partition_date_utc", $"prior_visit_call_date"))
   .withColumn("form_factor", upper(coalesce($"care_events_visit_form", upper(trim($"prior_visit_form")), lit("UNKNOWN"))))
   .withColumn("visit_type", coalesce($"care_events_visit_type", $"prior_visit_visit_type", lit("UNKNOWN")))
   .withColumn("app_type", coalesce($"care_events_app_type", $"prior_visit_app_type", lit("UNKNOWN")))
   ...
   .select($"visit_type", $"app_type" , $"form_factor", $"calls_with_visit",  $"authenticated_visits", $"call_date"
     )
...
//   the others have been needed at some point.  Feel free to add more commented
//   ones if you end up needing them.
cs_prodmonthly_cir_table.sort(col("call_date").desc,col("visit_type"),col("app_type"),col("form_factor")).show(200,false)
// steve_visit_rate_4calls.filter($"visit_type"==="SPECMOBILE").show(10,false)

/////////////////////////////// Write temp hack cs_call_in_rate//////////////////////////////
cs_prodmonthly_cir_table.createOrReplaceTempView("prodmonthlycir")
val prodmonthlycirtablename="cs_prodmonthly_call_in_rate_source"
println(s"writing ${daspDb}.${prodmonthlycirtablename} for data between ${overwriteDate} and ${endDate}")
spark.sql(
   s"""INSERT OVERWRITE TABLE ${daspDb}.${prodmonthlycirtablename} PARTITION(call_date)
     | SELECT visit_type,
     | app_type,
     | form_factor,
     | calls_with_visit,
     ...
     | call_date
     | FROM prodmonthlycir
     | where (visit_type != 'UNKNOWN'
     ) DISTRIBUTE BY call_date""".stripMargin
   )

   ////////////////////////////////////////////////////////////////////////////////
   //////////   End Temporary hack for prodmonthly changes   //////////////////////
   ////////////////////////////////////////////////////////////////////////////////
...
//    Feel free to add more commented show()s ones if you end up needing them.
        steve_calls_with_visits.show(10,false)
//        steve_calls_with_visits.filter($"primary_auto_disposition_issue"!=="null").filter($"primary_manual_disposition_issue"!=="null").show(10,false)
 ...
 val cs_visit_rate_call_data = (
   newCallData
    ...
    .agg(countDistinct(when($"enhanced_account_number" === lit(false)
      ...
      , countDistinct(when($"encrypted_padded_account_number_256" =!= "+yzQ1eS5iRmWnflCmvNlSg==" //"UNKNOWN"
 ...
 cs_visit_rate_steve_care_events
  ...
  .join(cs_visit_calls_with_prior_visit,
   ...
   .select($"acct_agent_mso", $"visit_type", $"customer_type" , $"calls_with_visit", $"validated_calls", $"distinct_call_accts"
     , $"handled_calls", $"distinct_visit_accts", $"authenticated_visits", $"call_date"
     )
     .where($"call_date" >= overwriteDate.toString
        && $"call_date" <= endDate.toString)
      )
...
spark.sql(
   s"""INSERT OVERWRITE TABLE ${daspDb}.${callRateTableName} PARTITION(call_date)
     ...
     | authenticated_visits,
     | call_date FROM rate4calls
     | where (visit_type != 'UNKNOWN'
     | and customer_type != 'UNMAPPED'
     | and acct_agent_mso in ('TWC','CHR','BHN')) DISTRIBUTE BY call_date""".stripMargin
   )
