DROP TABLE IF EXISTS TEST_TMP.ERR_TO_DATE;
---Create table to capture error metrics for an account from beginning to date.

CREATE TABLE TEST_TMP.ERR_TO_DATE AS
...
AND partition_date_denver BETWEEN  date_add('${run_date}', -6) AND '${run_date}'
...
DROP TABLE IF EXISTS TEST_TMP.ACTIVITY_TO_DATE;

-- Counts guide launch attempts, launch errors, and active days on zodiac/adobe

CREATE TABLE TEST_TMP.ACTIVITY_TO_DATE AS
...
WHERE partition_date_denver BETWEEN  date_add('${run_date}', -6) AND '${run_date}'
AND account_type = 'SUBSCRIBER'
GROUP BY partition_date_denver, account_number ) d
GROUP BY account_number;

--- Combines activity and error to date values
DROP TABLE IF EXISTS TEST_TMP.ERRORS_AND_ACTIVITY_TO_DATE;
CREATE TABLE TEST_TMP.ERRORS_AND_ACTIVITY_TO_DATE AS
SELECT activity.partition_date_denver,
       ...
       err.dvr_error_count,
       err.vod_error_count
FROM TEST_TMP.ACTIVITY_TO_DATE activity
FULL OUTER JOIN TEST_TMP.ERR_TO_DATE err
ON err.partition_date_denver = activity.partition_date_denver
AND err.account_number = activity.account_number;

--- drop table cleanup
DROP TABLE TEST_TMP.ACTIVITY_TO_DATE;
DROP TABLE TEST_TMP.ERR_TO_DATE;
--- Capture only Households that are currently deployed as of partition_date_denver

DROP TABLE IF EXISTS TEST_TMP.ACTIVITY_ERRS_IS_CURRENT;

CREATE TABLE TEST_TMP.ACTIVITY_ERRS_IS_CURRENT AS
...
FROM TEST_TMP.ERRORS_AND_ACTIVITY_TO_DATE activity
JOIN
...
INSERT INTO TABLE TEST.ACTIVITY_ERRORS_WEEKLY PARTITION(partition_date_denver)
...
SELECT activity.account_number,
       ...
       activity.vod_error_count,
       partition_date_denver
FROM TEST_TMP.ACTIVITY_ERRS_IS_CURRENT activity
JOIN
(SELECT account__number_aes256,
...
DROP TABLE TEST_TMP.ACTIVITY_ERRS_IS_CURRENT;
