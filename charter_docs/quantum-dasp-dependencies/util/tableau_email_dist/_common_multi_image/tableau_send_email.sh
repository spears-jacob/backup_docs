...
if [ "$RUN_DATE" != "" ]; then
  ...
  "
  data_cnt=$(hive -e "set hive.cli.print.header=false; USE $ENVIRONMENT; SELECT COUNT($report_table_counted_field) FROM $report_table WHERE $report_table_date_field = '$data_date' ;");
...
if [ ${data_cnt[0]} -gt 0 ]; then
  ...
  #       This requires a view that shows the date, the number of metrics, and
  #       a user-defined-in-Tableau threshold that sets the expectation to test
  #       against for the Total number of metrics.
  ...
  /usr/local/bin/tabcmd get "/views/$tableau_workbook/$tableau_metric_checking_view.csv"
  ...
  # command.1=/usr/local/bin/tabcmd get "/views/ASPDailyMetrics_Consolidated_Draft/MetricCheckingTotal.csv"

  #  The metric checking view used in this case is shown below
  #
  #  → cat MetricCheckingTotal.csv
  #
  #  Date_Denver,Does_Threshold_Match,Metrics_Count,Threshold
  #  2018-03-13,True,75,75
  #  2018-03-12,True,75,75
  #  2018-03-11,True,75,75
  #  2018-03-10,True,75,75
  #  2018-03-09,False,64,75
  #  2018-03-08,True,75,75
  #  2018-03-07,True,75,75
  ...
  export SecondLineOfCheckingView=$(cat $tab_ck_vw | sed -n '2 p')
  ...
  for index in "${!metric_checking_array[@]}"
    ...
    export TotalMetricCount="${metric_checking_array[2]}"
    ...
    if [ "${metric_checking_array[0]}" == "$data_date" ]; then echo "
      The date found in the Tableau report matches the data date of $data_date.";
    else
      TableuMetricCheckingIssue="The date found in the Tableau report does not match the data date of $data_date."
    fi

    if [ "${metric_checking_array[1]}" == "True" ]; then echo "
      The number of metrics found in the Tableau report matches the threshold as evaluated by the tableau report.";
    else
      TableuMetricCheckingIssue="The number of metrics found in the Tableau report does not match the threshold, as evaluated by the tableau report."
    fi
    ...
    if [ "${metric_checking_array[0]}" == "$data_date" ] && [ "${metric_checking_array[1]}" == "True" ] && [ "$TotalMetricCount" == "$ThresholdMatch" ]; then echo "
      ...
      if [[ -f "$index.png" ]]; then  echo "PNG is downloaded: $tableau_output_filename"
