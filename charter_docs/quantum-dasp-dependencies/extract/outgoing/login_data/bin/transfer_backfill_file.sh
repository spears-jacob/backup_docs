...
if [ "$RUN_DATE" != "" ]; then
  ...
  historical_date=$(date -d "$RUN_DATE 400 days ago" +%Y-%m-%d)
else
  echo "
...
if [ "$login_data_feed" -lt 1 ]; then
  ...
  exit 1
    ...
    FILE_NAME="$TABLE_NAME"_${extract_run_date}_historical
    echo "Writing to $FILE_NAME"

    mkdir -p ${LOCAL_DIR}
     hive -f ../src/create_historical_file_ccpa.hql --hiveconf min_date=$historical_date >> ${LOCAL_DIR}/$FILE_NAME.tsv
