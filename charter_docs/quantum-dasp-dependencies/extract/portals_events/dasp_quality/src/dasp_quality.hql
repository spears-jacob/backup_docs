...
SELECT
  ...
  metric_value,
  '' as duration_bucket,
  '' as page_load_time_bucket,
  '' as login_failure_bucket,
  denver_date
FROM
(
  SELECT
...
FROM (
  SELECT
    ...
    IF(has_successful_login,
       login_duration_bucket,0) AS login_duration_bucket,
    IF(has_successful_login,
       page_load_time_bucket,0) AS page_load_time_bucket,
    IF(has_successful_login,
       login_failure_bucket,0) AS login_failure_bucket
...
FROM (
  ...
  IF(has_successful_otp AND NOT has_more_than_allowed_failured_otps,
     transaction_duration_min_bucket,0) AS duration_bucket,
  IF(has_successful_otp AND NOT has_more_than_allowed_failured_otps,
     page_load_sec_bucket,0) AS page_load_time_bucket
