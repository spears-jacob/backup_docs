...
INSERT OVERWRITE TABLE ${env:ENVIRONMENT}.app_figures_downloads partition (partition_date_denver)
select app_details.product_id as product_id,
  product_name,
  developer,
  icon,
  vendor_identifier,
...
INSERT OVERWRITE TABLE ${env:ENVIRONMENT}.app_figures_ranks partition (partition_date_denver)
select distinct
  regexp_replace(product_id, "\\..*$", "") AS product_id,
  country,
  category_store,
